...
static void fmt_append_raw(buf *buf, fmt_spec spec, str s) {
    unused(spec);
    if (buf->v) {
        u64 copy_len = max(min(s.len, buf->cap - buf->len), 0);
...
static void fmt_append_f32(buf *buf, fmt_spec spec, f32 v) {
    if (spec.hex || spec.hexpad) {
        fmt_append_u32(buf, spec, f32_as_u32(v));
...
static void fmt_append_f64(buf *buf, fmt_spec spec, f64 v) {
    if (spec.hex || spec.hexpad) {
        fmt_append_u64(buf, spec, f64_as_u64(v));
...
static u64 fmt_args(u8 *out, u64 cap, args args) {
    span span = make_span(u8, out, cap);
    buf buf = make_buf(span, 0);
...
static void print_args(args args) {
    u8 scratch[4096];
    u64 len = fmt_args(scratch, sizeof(scratch) - 1, args);
    scratch[len] = '\0';
#ifdef NDEBUG
...
static str arena_push_fmt_args(arena *arena, args args) {
    u64 len = fmt_args(0, 0, args);
    str ret = { 0 };
    ret.v = arena_push_size_nz(arena, len + 1, 0);
    if (ret.v) {
...
static void fmt_pad_until(buf *buf, u64 pad_len) {
    if (pad_len > buf->len) {
        u64 add_chars = pad_len - buf->len;
        for_n (u64, i, add_chars) {
