...
static char_flags u8_get_char_flags(u8 c) {
    static_assert(countof(char_flags_table) >= U8_MAX);
...
static u64_result str_idx_of_str_nocase(str a, str b) {
    u64_result ret = { 0 };
...
static u64_result str_idx_of_any(str s, str any) {
    u64_result ret = { 0 };
...
static str str_chop_by_delim(str *s, str delim) {
    ...
    u64_result idx = str_idx_of_str(*s, delim);
    if (idx.found) {
        ret = substr_len(*s, 0, idx.v);
        *s  = substr_idx(*s, idx.v + delim.count);
    } else {
        ret = *s;
        *s = str("");
    }
...
static bool str_starts_with_str(str s, str prefix) {
    u64_result result = str_idx_of_str(s, prefix);
    bool ret = result.found && result.v == 0;
    return ret;
}
...
static bool str_ends_with_str(str s, str suffix) {
    ...
    u64_result result = str_idx_of_str(s, suffix);
    bool ret = result.has_value && result.v == s.count - suffix.count;
    return ret;
}
...
static i32 str_cmp_nocase_(const str *a, const str *b) {
    return str_cmp_nocase(*a, *b);
}

static void str_sort_nocase(str *strings, u64 count) {
    qsort(strings, count, sizeof(str), str_cmp_nocase_);
}
...
static void fuzzy_match_list_insert_after(fuzzy_match_list *list, fuzzy_match_node *after, u64_range range, arena *arena) {
...
static fuzzy_match_list fuzzy_match_list_from_str(str haystack, str_list needles, arena *arena) {
    ...
    for_list (str_node, n, needles) {
        ...
        while (at < haystack.len) {
            ...
            u64_result idx = str_idx_of_str_nocase(remaining, n->v);
            if (idx.found) {
                u64_range found_range = u64_range(at + idx.v, at + idx.v + n->v.len);
