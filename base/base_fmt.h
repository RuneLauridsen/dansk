...
static u64 fmt_args(u8 *out, u64 cap, args args); // NOTE(rune): If out is null, returns number if bytes that would have been written.
...
#define arena_push_fmt(arena, ...)    arena_push_fmt_args(arena, argsof(__VA_ARGS__))

#define dumpvar(expr) println("% = %(lit)", #expr, (expr))
#define dumpspan(expr) do {                                 \
    for_range (ix, _i, 0, (expr).count) {                   \
        println("%[%] = %(lit)", #expr, _i, (expr).v[_i]);  \
    }                                                       \
} while(0)
